...
function formReset()
		{
			...
			document.getElementById('acct').checked=false;
			...
			if(paramsStart != -1){
				...
				if(tokenStart	!= -1){
					...
					if(delimiterPos ==	-1){
						...
						paramToEnd= paramToEnd.substr(0,	delimiterPos);
					}
				}
			}
			//document.write(paramToEnd)
			var dropdown=document.getElementById('CountySelect');
			dropdown.selectedIndex=paramToEnd-1;
		}
		
		function openDefWindow(URL){
			URL="rp_def.html"+URL;
				window.open(URL,"Glossary","scrollbars=yes,resizable=yes,width=400,height=300");
			}		
